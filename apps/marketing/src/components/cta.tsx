...
/** Shared button class strings, so the router's <Link> can be styled like these anchors too. */
...
export function PrimaryLink({ className, ...props }: AnchorHTMLAttributes<HTMLAnchorElement>) {
  return <a className={cn(primaryClass, className)} {...props} />;
}

export function OutlineLink({ className, ...props }: AnchorHTMLAttributes<HTMLAnchorElement>) {
  return <a className={cn(outlineClass, className)} {...props} />;
}
